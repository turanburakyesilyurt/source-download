...
(function (root) {
  ...
  async function combine(manifestUrl, fetchers, onProgress) {
    ...
    for (let i = 0; i < parsed.segments.length; i++) {
      const seg = await fetchers.bytes(parsed.segments[i].url);
      if (!seg) throw new Error('HLS: segment ' + (i + 1) + ' of ' + parsed.segments.length + ' could not be downloaded');
      parts.push(seg);
      if (onProgress) onProgress((i + 1) / parsed.segments.length);
    }
    return { bytes: concatBytes(parts), mime, segments: parsed.segments.length, live: !parsed.endlist };
  }
