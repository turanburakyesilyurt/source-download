...
Google Chrome DevTools、サイドパネル、ツールバーポップアップにネイティブ統合された機能をご覧ください。
...
Chrome F12 開発者ツール（DevTools）専用エンジニアリングパネル、ブラウザ領域を狭めずに常駐できるサイドパネル（Side Panel）、そして手軽にアクセスできるツールバーポップアップの3つの柔軟な作業環境を搭載。訪問中のあらゆるWebページがバックグラウンドで読み込む膨大なリソースを徹底的に検出・分類し、高解像度メディアやSVGベクター、HLS動画配信ストリーム、動的DOMテーブル、Webフォント、APIレスポンスに至るまで、すべてのデータを詳細な技術仕様とともに瞬時に抽出し、個別ダウンロードや整理されたZIP/ZIP64アーカイブとして一括保存できます。
...
- **contextMenus:** 右クリックメニューにクイック操作（要素消去、範囲キャプチャ、全画面キャプチャ）を追加するために使用。
- **sidePanel:** メインの閲覧画面を縮小させることなく、リソースの一覧や操作パネルを並行表示するために使用。
...
### 1. サイドパネル（Side Panel）によるマルチタスク作業フロー

Chrome 114以降で標準搭載されたブラウザサイドパネルAPIに完全対応：
- **メインビューポートの完全保護:** 従来のポップアップUIとは異なり、ページ上の操作やクリックを行ってもパネルが勝手に閉じることはありません。
- **リアルタイムアセットストリーム:** ページを閲覧しながら、新しくロードされる画像、動画、フォント、API通信がサイドパネル内のリストへリアルタイムに追加されます。
- **即時プレビューと個別ダウンロード:** 気になるアセットをクリックするだけで、サイドパネル下部で即座に拡大プレビューやメタデータ確認、単体ダウンロードが実行可能です。
...
3. 任意のWebページで `F12`（macOSは `Cmd+Option+I`）を押し、「**Source Download**」タブに切り替えるか、ツールバーからサイドパネルを開きます。
